...
import { FiMenu, FiX, FiSettings, FiFileText, FiMail, FiPrinter, FiCheckSquare, FiInfo, FiCreditCard, FiMessageSquare } from 'react-icons/fi';
...
const EventSettingsLayout: React.FC = () => {
  ...
  // Enhanced current step detection (for reference, though ArrowStepper handles this)
  ...
  // Type guard to ensure eventId is defined
  if (!eventId) {
    return <div className="w-full h-full flex items-center justify-center text-red-600">Event ID is missing</div>;
  }
  ...
  return (
    <div className="relative w-full h-full flex">
        ...
        <div className={`overflow-hidden h-full flex flex-col transition-opacity ${isNavOpen ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="px-6 py-5 flex items-center justify-between border-b border-gray-100 bg-gray-50">
            <div>
              ...
              {currentEvent && <p className="text-sm text-gray-500 mt-1">{currentEvent.title}</p>}
            </div>
            <button
          ...
          <div className="px-4 py-4 border-t border-gray-100 bg-gray-50">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Current Step</div>
            <div className="text-sm font-medium text-blue-600">{eventSteps[currentStep].label}</div>
            <div className="mt-2 flex items-center">
              {eventSteps.map((_, index) => (
                <div
                  key={index}
                  className={`h-1 rounded-full mx-0.5 ${
                    index === currentStep ? 'w-6 bg-blue-600' : index < currentStep ? 'w-4 bg-blue-400' : 'w-2 bg-gray-300'
                  }`}
                />
              ))}
            </div>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-gray-50">
              ...
              {eventSteps.map((_, index) => (
                <div
                  ...
                  className={`h-2 w-2 rounded-full mx-1 ${
                    index === currentStep ? 'bg-blue-600' : index < currentStep ? 'bg-blue-400' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
        ...
        {/* Navigation Buttons */}
        <NavigationButtons eventId={eventId} steps={eventSteps} />
      </main>
    </div>
  );
};
