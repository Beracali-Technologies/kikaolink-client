...
import { StandardFieldsPanelProps } from './StandardFieldsPanel.types';

const StandardFieldsPanel: React.FC<StandardFieldsPanelProps> = ({
  allStandardFields,
  activeFields,
  onToggle,
}) => {
    ...
    <div>
      ...
      <div className="space-y-3">
        {allStandardFields.map((standardField) => {
          ...
          const isChecked = activeFields.some(
            (af) => af.systemName === systemName
          );
          ...
          return (
            <div key={systemName} className="flex items-center">
              ...
              />
              <label
                htmlFor={`sf-${systemName}`}
                className="ms-3 block text-sm font-medium text-gray-800"
              >
                {label}
              </label>
            </div>
